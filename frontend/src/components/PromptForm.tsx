    ...
    <header className="absolute top-4 left-6 right-6 z-20 flex items-center justify-between pointer-events-none">
      ...
      >
        ...
        <input
          ...
          className="bg-transparent text-sm text-white placeholder-slate-400 focus:outline-none w-full px-2 py-1 font-sans disabled:opacity-50"
        />
        <button
          type="submit"
          disabled={isLoading || !promptText.trim()}
          className="bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-black font-semibold text-xs px-4 py-2 rounded-lg transition-all shadow-md shadow-cyan-500/20 shrink-0 flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isLoading ? (
            <>
